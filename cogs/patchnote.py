...
# patch note commands need to be changed as pagination.
...
class PatchNote(commands.GroupCog, name='pn'):
    ...
    async def alu_process(self):
        ...
        if not content:
            self.logger.error('아스팔트 패치노트 - newsArticles를 포함하는 스크립트를 검색 실패.')
            return
        ...
        if search:
            ...
            self.logger.error('아스팔트 패치노트 - 필요한 데이터를 추출할 수 없음.')
    ...
    async def delete_old_patchnote(self):   
        ...
        try:
            deleted = await self.pnlog.delete_many({"date" : {"$lt" : threshold_date}})
            if deleted.deleted_count > 0:
                self.logger.warning(f"{deleted.deleted_count}개의 오래된 패치노트 삭제 완료")
        except Exception:
            pass
